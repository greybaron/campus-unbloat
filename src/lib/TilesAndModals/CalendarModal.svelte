	...
	let options = {
		...
		eventContent: function(arg: EventContentArg) {
			let eventId = 'custom-event-' + arg.event.id;
  		 	let customHtml = 
				'<div id="' + eventId + '" class="flex flex-col items-center justify-center h-full md:space-y-0 -space-y-5 cursor-pointer transition-transform transform hover:scale-105">' +
				'<div class="flex justify-center"> <p class="md:text-xl text-sm">' + arg.event.title + ' </p> </div>'+
				'<div class="flex justify-center"> <p class="md:text-sm md:visible invisible">' + (arg.event.start?.getHours().toString().padStart(2, '0') + ":" + arg.event.start?.getMinutes().toString().padStart(2, '0') + " - " + arg.event.end?.getHours().toString().padStart(2, '0') + ":" + arg.event.end?.getMinutes().toString().padStart(2, '0')) + ' </p> </div>' +
				'</div>';
...
<DashboardModal bind:parent title="Kalender">
	<div class="radio-group-container w-full  flex justify-center pb-2">
		<RadioGroup bind:group={view} active="variant-filled-primary" hover="hover:variant-soft-primary">
			<RadioItem value="week" bind:group={view} name="calendar-view" label="Wochenübersicht">Wochenübersicht</RadioItem>
		  	<RadioItem value="day" bind:group={view} name="calendar-view" label="Tagesansicht">Tagesansicht</RadioItem>
		</RadioGroup>
	</div>
		...
		<div class="space-y-3 pr-1 pl-1 pt-2 w-full pb-1 flex-col items-center">
			...
			{:else}
				{#each todaysEvents as {start, end, title, room, instructor, remarks, color}}
					<div class="flex flex-row justify-start rounded-xl space-x-1 pr-4 bg-surface-200-700-token">
